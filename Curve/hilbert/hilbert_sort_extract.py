...
for n in range(0,max_level+1):
    queue = [1] * (1<<(n*2))
    with open(save_root + 'hilbert2d_sort^'+str(n)+'_1.txt', 'w') as f:
    ...
    if n <= 4:
        hilbert_vis(queue_2d, save_root + 'hilbert2d^'+str(n)+'_1.png')

    with open(save_root + 'hilbert2d_queue^'+str(n)+'_1.txt', 'w') as f:
        for i in queue:
            f.write(str(i) + ' ') 
    f.close()
