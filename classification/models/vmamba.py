...
hilber2d_index_shift_list = []
hilber2d_index_reverse_shift_list = []
    
def hilbert_presolve(size,save_path):
    save_path += '/hilbert'
    Path(save_path).mkdir(parents=True, exist_ok=True)
    global hilber2d_index_list, hilber2d_index_reverse_list, hilber2d_index_shift_list, hilber2d_index_reverse_shift_list
    
    for n in range(9):
        hilber2d_index_list.append([])
        hilber2d_index_reverse_list.append([])
        hilber2d_index_shift_list.append([])
        hilber2d_index_reverse_shift_list.append([])

        if not 1 << n >= size: continue

        for i in range(4):
            index = open('../Curve/hilbert/save_v0/hilbert2d_sort^'+str(n)+'_'+str(i)+'.txt', 'r').read()
            index = index.split(' ')[:-1]
            hilbert_index = torch.tensor([int(x) for x in index])
            hilbert_queue_standard = torch.argsort(hilbert_index) 

            # 裁剪
            hilbert_queue = [x // (1<<n) * size + x %(1<<n) for x in hilbert_queue_standard if x//(1<<n) < size  and x%(1<<n) < size]
            hilbert_queue_2d = [[x.item()//size, x.item()%size] for x in hilbert_queue]
            #hilbert_vis(curve=hilbert_queue_2d, save_path=os.path.join(save_path, 'clip_hilbert2d^'+str(n)+'_'+str(i)+'.png'))
            with open(os.path.join(save_path, 'clip_hilbert2d_queue^'+str(n)+'_'+str(i)+'.txt'), 'w') as f:
                f.write(str(hilbert_queue_2d))
            hilbert_queue = torch.tensor(hilbert_queue).cuda()

            # 裁剪并shift
            hilbert_queue_shift = [(x // (1<<n) + 1 ) % size * size + (x %(1<<n) + 1) % size for x in hilbert_queue_standard if x//(1<<n) < size  and x%(1<<n) < size]
            hilbert_queue_shift_2d = [[x.item()//size, x.item()%size] for x in hilbert_queue_shift]
            #hilbert_vis(curve=hilbert_queue_shift_2d, save_path=os.path.join(save_path, 'clip_hilbert2d_shift^'+str(n)+'_'+str(i)+'.png'))
            with open(os.path.join(save_path, 'clip_hilbert2d_shift_queue^'+str(n)+'_'+str(i)+'.txt'), 'w') as f:
                f.write(str(hilbert_queue_shift_2d))
            hilbert_queue_shift = torch.tensor(hilbert_queue_shift).cuda()

            hilbert_index = torch.argsort(hilbert_queue)
            hilbert_index_record = [x.item() for x in hilbert_index]
            with open(os.path.join(save_path, 'clip_hilbert2d_sort^'+str(n)+'_'+str(i)+'.txt'), 'w') as f:
                f.write(str(hilbert_index_record))

            hilbert_index_shift = torch.argsort(hilbert_queue_shift)
            hilbert_index_shift_record = [x.item() for x in hilbert_index_shift]
            with open(os.path.join(save_path, 'clip_hilbert2d_shift_sort^'+str(n)+'_'+str(i)+'.txt'), 'w') as f:
                f.write(str(hilbert_index_shift_record))

            hilber2d_index_list[-1].append(hilbert_index)
            hilber2d_index_reverse_list[-1].append(hilbert_queue)
            
            #hilber2d_index_list[-1].append(hilbert_queue)
            #hilber2d_index_reverse_list[-1].append(hilbert_index)
            
            hilber2d_index_shift_list[-1].append(hilbert_index_shift)
            hilber2d_index_reverse_shift_list[-1].append(hilbert_queue_shift)
...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        log_level = int(math.ceil(math.log2(H)))
...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln", **kwargs):
        ...
        if len(hilber2d_index_list) == 0:
            hilbert_presolve(size=16, save_path='work_dirs/')
